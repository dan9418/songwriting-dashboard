...
import { getPrimaryDirectArtistImage } from "@/lib/cloudflare/images";
...
async function assertArtistExists(artistSlug: string): Promise<void> {
  const rows = await queryD1<ArtistExistsRow>(
    `
    SELECT slug
    FROM artists
    WHERE slug = ?
...
function buildArtistImagePath(artistSlug: string, extension: string): string {
  return `images/${artistSlug}${extension}`;
}
...
export async function POST(
  ...
  try {
    ...
    const nextImageSlug = artistSlug;
    const nextImagePath = buildArtistImagePath(artistSlug, extension);
    const currentDirectImage = await getPrimaryDirectArtistImage(artistSlug);
    const canonicalImage = await getImageRow(nextImageSlug);

    if (canonicalImage) {
      const refs = await getImageReferenceCounts(nextImageSlug);
      const expectedArtistRefs = currentDirectImage?.imageSlug === nextImageSlug ? 1 : 0;
      if (refs.artistRefs > expectedArtistRefs || refs.projectRefs > 0 || refs.trackRefs > 0) {
    ...
    try {
      ...
      await queryD1(
        `
        INSERT INTO artist_images (artist_slug, image_slug)
        VALUES (?, ?)
        ON CONFLICT(artist_slug, image_slug) DO NOTHING;
        `,
        [artistSlug, nextImageSlug]
      );

      if (currentDirectImage && currentDirectImage.imageSlug !== nextImageSlug) {
        await queryD1(`DELETE FROM artist_images WHERE artist_slug = ? AND image_slug = ?;`, [
          artistSlug,
...
export async function DELETE(
  ...
  try {
    ...
    if (!currentDirectImage) {
      throw notFound(`Artist does not have a direct image: ${artistSlug}`);
    }

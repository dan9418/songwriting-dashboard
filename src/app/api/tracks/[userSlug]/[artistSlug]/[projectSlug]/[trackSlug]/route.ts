...
export async function GET(
  _: Request,
  context: {
    params: Promise<{ userSlug: string; artistSlug: string; projectSlug: string; trackSlug: string }>;
  }
) {
  try {
    const { userSlug, artistSlug, projectSlug, trackSlug } = await context.params;
...
export async function PATCH(
  ...
  try {
    ...
    const current = await getTrack(userSlug, artistSlug, projectSlug, trackSlug);
    const nextData = enforceTrackAudioNaming({ ...current.data, ...body.data });
    assertSlugMatch(nextData.slug, trackSlug);
    await saveTrack(userSlug, artistSlug, projectSlug, trackSlug, nextData, body.content ?? current.content);
    const entity = await getTrack(userSlug, artistSlug, projectSlug, trackSlug);
    return ok(entity);
  } catch (error) {
    return apiErrorResponse(error);
...
export async function DELETE(
  ...
  try {
    const { userSlug, artistSlug, projectSlug, trackSlug } = await context.params;
    const existed = await deleteTrack(userSlug, artistSlug, projectSlug, trackSlug);
